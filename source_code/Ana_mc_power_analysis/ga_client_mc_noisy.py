#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# MountainCar GA Client (Discrete): Energy Shaping + Batch Run (Seeds 101-105) IN NOISY ENVIRONMENT
...
# ==========================================
# [NEW] Environment Wrapper (Pure NumPy for Multiprocessing Safety)
# ==========================================
class VisuallyClutteredWrapper(gym.Wrapper):
    def __init__(self, env, gaussian_std=25.0, add_clutter=True):
        ...
        # MountainCar native resolution is 600x400 (W=600, H=400)
    ...
    def render(self):
        ...
        if self.add_clutter:
            ...
            # Pure numpy slicing to avoid cv2 segmentation faults in multiprocessing
...
class RPCClient:
    ...
    def reset(self):
        if self.conn: self.conn.send(("reset", None)); _ = self.conn.recv()
...
def evaluate_individual(args):
    ...
    try:
        ...
        # [MODIFIED] Inject visual noise wrapper
...
def run_ga(args):
    ...
    # [MODIFIED] Added 'noisy' tag to directory name
    run_dir = os.path.join(args.outdir, f"ga_mc_discrete_noisy_{ts}_seed{args.global_seed}")
    ...
    print(f"MountainCar GA (Discrete - NOISY) started. Global Seed: {args.global_seed}. Logs: {run_dir}")
...
if __name__ == "__main__":
    ...
    # [MODIFIED] Default outdir changed to runs_mc_discrete_noisy
    parser.add_argument("--outdir", default="runs_mc_discrete_noisy")
