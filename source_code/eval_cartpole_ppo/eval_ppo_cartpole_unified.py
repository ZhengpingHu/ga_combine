...
MODELS_NOISY = {
    101: "./ppo_3m_noisy_results_seed_101/final_ppo_model.zip",
    102: "./ppo_3m_noisy_results_seed_102/final_ppo_model.zip",
    103: "./ppo_3m_noisy_results_seed_103/final_ppo_model.zip",
    104: "./ppo_3m_noisy_results_seed_104/final_ppo_model.zip",
    105: "./ppo_3m_noisy_results_seed_105/final_ppo_model.zip"
}
...
def evaluate(train_type, test_type, outdir, n_stack):
    ...
    # 1. 保存 CSV 数据
    filename_base = f"ppo_cartpole_{train_type}_to_{test_type}"
    ...
    # 根据数据动态调整 Y 轴显示范围
    y_min = df['Reward'].min()
    view_min = max(0, y_min - (500-y_min)*0.1 if y_min < 500 else 495)
