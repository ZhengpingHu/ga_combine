...
# Modified for Multi-Run (Seeds 101-105) + NOISY ENVIRONMENT INJECTION
...
# (Exactly the same as used in PPO for fair comparison)
# ==========================================
...
def run_ga(args):
    ...
    # Mark as noisy in directory name
    run_dir = os.path.join(args.outdir, f"ga_noisy_cartpole_{ts}_seed{args.global_seed}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--processes", type=int, default=16) # Set to 16 by default to match your system
    ...
    parser.add_argument("--outdir", default="runs_noisy_cartpole") # Modified output dir
    ...
    print(f"Starting Batch Experiment: {len(seeds_to_run)} runs IN NOISY ENVIRONMENT.")
    print(f"Seeds: {seeds_to_run}")
    
    for run_idx, seed in enumerate(seeds_to_run):
        print(f"\n{'='*60}")
        print(f"STARTING NOISY RUN {run_idx+1}/{len(seeds_to_run)} | SEED: {seed}")
        print(f"{'='*60}\n")
        ...
        print(f"\n{'='*60}")
        print(f"FINISHED NOISY RUN {run_idx+1}/{len(seeds_to_run)} | SEED: {seed}")
        print(f"{'='*60}\n")