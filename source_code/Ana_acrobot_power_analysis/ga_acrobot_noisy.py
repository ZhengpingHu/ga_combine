#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Acrobot GA Client: Fixed Linear Topology + Proportional Fitness Sharing + Dual Refresh (NOISY ENVIRONMENT)
...
class VisuallyClutteredWrapper(gym.Wrapper):
    ...
    def render(self):
        ...
        if self.add_clutter:
            ...
            # 使用纯 NumPy 切片防止 cv2.resize 导致的底层崩溃
...
def run_ga(args):
    ...
    run_dir = os.path.join(args.outdir, f"ga_acrobot_noisy_{ts}") # [修改] 自动打上 noisy 标签
    ...
    print(f"Acrobot (NOISY) Training started. Logs at: {run_dir}")
...
if __name__ == "__main__":
    ...
    # [修改] 默认输出文件夹改为 runs_acrobot_noisy 避免混淆
    parser.add_argument("--outdir", default="runs_acrobot_noisy")
