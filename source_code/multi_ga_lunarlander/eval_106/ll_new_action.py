#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# LunarLander Dual Analysis: Probabilistic Workspace (Alpha-blended) & Trajectory
# IEEE CEC Edition
...
# Note: Action 0 (Do Nothing) is now represented by Transparency, 
# but we keep a light gray for the legend.
ACTION_COLORS = {
    0: '#E0E0E0',  # Light Gray (For Legend Only)
    1: '#1E88E5',  # Blue  -> Left Engine
    2: '#E53935',  # Red   -> Main Engine
    3: '#43A047'   # Green -> Right Engine
}
ACTION_LABELS = {
    0: 'Do Nothing (Transparent)',
    1: 'Left Engine',
    2: 'Main Engine',
    3: 'Right Engine'
}
...
def generate_clean_workspace_grid(model):
    print("Generating probabilistic grid states for clean global workspace...")
    data_log = []
...
def create_standard_legend():
    legend_elements = []
    for act in range(4):
        # We use a dashed/hollow marker for Do Nothing in the legend to imply transparency
        if act == 0:
            legend_elements.append(
                mlines.Line2D([0], [0], marker='s', color='w', markerfacecolor='none', 
                              markeredgecolor='gray', markersize=8, label=ACTION_LABELS[act])
            )
        else:
            legend_elements.append(
...
def plot_global_workspace_probabilistic(df_bg):
    print("\nGenerating Probabilistic Global Workspace Area Plot (Alpha-blended)...")
    fig, ax = plt.subplots(figsize=(4.5, 3.5), dpi=300)
    
    rgb_1 = np.array(mcolors.to_rgb(ACTION_COLORS[1]))
    rgb_2 = np.array(mcolors.to_rgb(ACTION_COLORS[2]))
    rgb_3 = np.array(mcolors.to_rgb(ACTION_COLORS[3]))
    
    # Calculate the total probability of active engines (P1 + P2 + P3)
    # The probability of Do Nothing (P0) becomes the transparency factor.
    # If P0 = 1, alpha = 0 (completely transparent). If P0 = 0, alpha = 1 (fully opaque).
    p_active = df_bg['Prob_1'] + df_bg['Prob_2'] + df_bg['Prob_3']
    
    # Avoid division by zero when calculating base color
    p_active_safe = np.where(p_active == 0, 1.0, p_active)
    
    # Calculate normalized RGB (excluding Do Nothing probability)
    r = (df_bg['Prob_1'] * rgb_1[0] + df_bg['Prob_2'] * rgb_2[0] + df_bg['Prob_3'] * rgb_3[0]) / p_active_safe
    g = (df_bg['Prob_1'] * rgb_1[1] + df_bg['Prob_2'] * rgb_2[1] + df_bg['Prob_3'] * rgb_3[1]) / p_active_safe
    b = (df_bg['Prob_1'] * rgb_1[2] + df_bg['Prob_2'] * rgb_2[2] + df_bg['Prob_3'] * rgb_3[2]) / p_active_safe
    
    # The alpha channel is strictly the sum of active action probabilities
    a = p_active.values
    
    rgba_colors = np.column_stack((r, g, b, a))
    
    # Plot using RGBA colors
    ax.scatter(
        df_bg['X'], df_bg['Y'], 
        c=rgba_colors, 
        s=25, marker='s', edgecolors='none', rasterized=True
    )

    ax.set_title("Probabilistic Policy Workspace Mapping", fontweight='bold')
    ...
    ax.legend(handles=create_standard_legend(), loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=2, frameon=False)
    
    pdf_path = os.path.join(OUTPUT_DIR, "fig_workspace_global_prob.pdf")
...
def plot_single_trajectory(df_traj):
    ...
    for act in sorted_actions:
        ...
        if len(subset) > 0:
            # For trajectory, we keep Action 0 as light gray dots to show where it did nothing
            color = ACTION_COLORS[act] if act != 0 else '#B0BEC5'
            ax.scatter(
                subset['X'], subset['Y'], 
                c=color, 
                s=12, zorder=4, edgecolors='none'
            )
...
if __name__ == "__main__":
    if not os.path.exists(MODEL_PATH):
        ...
        # 1. Plot probabilistic decision boundary (No RPC Required)
        df_background = generate_clean_workspace_grid(model)
        plot_global_workspace_probabilistic(df_background)
        
        # 2. Plot real trajectory (Requires YOLO RPC Server listening on port 6001)
        # If you don't want to run the server right now, comment the next two lines out:
