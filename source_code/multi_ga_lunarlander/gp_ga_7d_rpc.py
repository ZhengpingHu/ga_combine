#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# LunarLander GA Client: Proportional Fitness Sharing + Corrected Metrics + Multi-Seed Run (101-105)

import os
# Fix OMP conflict error before importing torch
...
class NNPolicy(nn.Module):
    def __init__(self, hidden=64):
        super().__init__()
        # Input size 7 (from YOLO state), Output 4 (Discrete actions)
...
def calculate_competitive_fitness(results_matrix: np.ndarray) -> np.ndarray:
    ...
    # Ensure all values are positive for proportional sharing
    if min_val <= 0:
    ...
    total_rewards_per_seed[total_rewards_per_seed == 0] = 1e-9 # Avoid division by zero
...
def run_ga(args):
    # Update random seeds for this run
    if args.global_seed: set_global_seed(args.global_seed)
    
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    # [Mod] Add seed to directory name
    run_dir = os.path.join(args.outdir, f"ga_lunar_{ts}_seed{args.global_seed}")
    ...
    print(f"Training started. Global Seed: {args.global_seed}. Logs: {run_dir}")
...
if __name__ == "__main__":
    ...
    for seed in target_seeds:
        print("\n" + "="*60)
        print(f"  EXECUTING RUN WITH GLOBAL SEED: {seed}")
        print("="*60 + "\n")
        
        # Set seeds for this specific run
        args.global_seed = seed
        args.pool_rng_seed = seed # Ensure env pool generation is also unique/isolated
        
        run_ga(args)
        
        # Pause to separate logs and prevent file conflicts
        time.sleep(2)
