...
# ==========================================
MODELS_CLEAN = {
    101: "./ppo_3m_clean_acrobot_results_seed_101/final_ppo_model.zip",
    102: "./ppo_3m_clean_acrobot_results_seed_102/final_ppo_model.zip",
    103: "./ppo_3m_clean_acrobot_results_seed_103/final_ppo_model.zip",
    104: "./ppo_3m_clean_acrobot_results_seed_104/final_ppo_model.zip",
    105: "./ppo_3m_clean_acrobot_results_seed_105/final_ppo_model.zip"
}

MODELS_NOISY = {
    101: "./ppo_3m_noisy_acrobot_results_seed_101/final_ppo_model.zip",
    102: "./ppo_3m_noisy_acrobot_results_seed_102/final_ppo_model.zip",
    103: "./ppo_3m_noisy_acrobot_results_seed_103/final_ppo_model.zip",
    104: "./ppo_3m_noisy_acrobot_results_seed_104/final_ppo_model.zip",
    105: "./ppo_3m_noisy_acrobot_results_seed_105/final_ppo_model.zip"
}
...
def evaluate(train_type, test_type, outdir, n_stack):
    ...
    # 1. 保存 CSV 数据
    filename_base = f"ppo_acrobot_{train_type}_to_{test_type}"
    ...
    # Acrobot 得分通常在 -500 到 0 之间
    y_min = df['Reward'].min()
    y_max = df['Reward'].max()
    
    # 设置底部界限，最差就是 -500 左右，留一点余量
    view_min = max(-500, y_min - 20)
    view_max = min(20, y_max + 20) if y_max < 0 else 20
...
def main():
    ...
    parser.add_argument("--outdir", default="eval_data_acrobot")
