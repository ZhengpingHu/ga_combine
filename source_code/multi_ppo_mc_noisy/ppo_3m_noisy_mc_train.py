...
# ==========================================
# 1. Pixel Wrapper + Energy Shaping (NOISY)
# ==========================================
class NoisyPixelMountainCar(gym.Wrapper):
    ...
    def step(self, action):
        ...
        # 如果回合结束，将最真实的原始分数注入 info 供 CSV 记录
    ...
    def _get_obs(self):
        ...
        if self.add_clutter:
            # 遮挡天空背景 (MountainCar 天空大多较亮，>200)
            bg_mask = np.all(frame > 200, axis=-1)
...
class Strict10kLoggerCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        # 提取真实的物理原始分数 (而非网络看到的 Shaped Reward)
...
if __name__ == "__main__":
    ...
    # MountainCar 每回合200步，给予 300万 步算是十分充裕的预算
    max_training_steps = 3_000_000 
    target_score = -110.0 
    
    print("=== Starting E2E PPO (3M Params) Tests on MOUNTAINCAR IN NOISY ENVIRONMENT ===")
    
    for run_seed in test_seeds:
        print(f"\n--- Running NOISY MountainCar Seed {run_seed} ---")
        seed_dir = f"ppo_3m_noisy_mc_results_seed_{run_seed}"
        ...
        vec_env = make_vec_env(
            ...
            vec_env_cls=SubprocVecEnv, wrapper_class=NoisyPixelMountainCar,
            env_kwargs={"render_mode": "rgb_array"}
        )
        vec_env = VecFrameStack(vec_env, n_stack=4)
        
        # 3M Parameter "Golden Ratio" Architecture
