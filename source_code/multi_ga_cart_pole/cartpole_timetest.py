#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# CartPole GA Client: Fixed Linear Topology + Proportional Fitness Sharing (Doc Page 13) + Dual Refresh
# Modified for Multi-Run (Seeds 101-105) + Performance Time Profiling
...
import pickle
import time  # [ADDED] For high-precision profiling
...
# Custom print function to ensure flushing
...
class SeedPortfolioManager:
    ...
    def update_and_refresh(self, results_matrix: np.ndarray, refresh_frac: float, refresh_direction: str, max_seed_age: int):
        ...
        num_replace = len(indices_to_replace)
        candidate_pool = [s for s in self.master_pool if s not in self.active_subset]
        
        if len(candidate_pool) < num_replace:
            num_replace = len(candidate_pool); indices_to_replace = list(indices_to_replace)[:num_replace]
...
def evaluate_individual(args):
    ...
    # [MODIFIED] Initialize profiling timers
    time_physics = 0.0
    time_render = 0.0
    time_rpc_infer = 0.0
    ...
    try:
        ...
        with RPCClient(rpc_host, rpc_port, authkey) as rpc:
            ...
            for t in range(max_steps):
                ...
                time_render += (time.perf_counter() - t0)
                
                # 2. Profile RPC network I/O + YOLO inference
                t1 = time.perf_counter()
                state = rpc.infer(frame_bgr)
                time_rpc_infer += (time.perf_counter() - t1)
                ...
                # 3. Profile simulation physics step
                t2 = time.perf_counter()
                obs, reward, done, truncated, info = env.step(act)
                time_physics += (time.perf_counter() - t2)
                
                total_reward += reward
                if done or truncated: break
        env.close()
        # [MODIFIED] Return timing metrics alongside results
        return pop_idx, seed_idx, total_reward, time_physics, time_render, time_rpc_infer
    except Exception:
        # [MODIFIED] Match expected return tuple size on failure
        return pop_idx, seed_idx, 0.0, 0.0, 0.0, 0.0

def calculate_competitive_fitness(results_matrix: np.ndarray) -> np.ndarray:
    if np.min(results_matrix) < 0:
        results_matrix = results_matrix - np.min(results_matrix)

    total_rewards_per_seed = results_matrix.sum(axis=0)
    total_rewards_per_seed[total_rewards_per_seed == 0] = 1e-9
    shared_fitness_matrix = results_matrix / total_rewards_per_seed
...
def plot_separated_curves(run_dir, df_history):
    ...
    plt.xlabel("Generation"); plt.ylabel("Shared Fitness"); plt.title("Optimization Metric (Page 13 Formula)")
    ...
    plt.xlabel("Generation"); plt.ylabel("Raw Reward"); plt.title("Population Absolute Performance")
...
def plot_final_violin(final_results_matrix, run_dir):
    try:
        ...
        plt.title('Final Population Generalization')
...
def run_ga(args):
    ...
    run_dir = os.path.join(args.outdir, f"ga_cartpole_{ts}_seed{args.global_seed}")
    ...
    for gen in range(1, args.generations + 1):
        ...
        # [MODIFIED] Generation-level time accumulators
        gen_time_physics = 0.0
        gen_time_render = 0.0
        gen_time_rpc = 0.0
        
        results_matrix = np.full((args.population, args.subset_k), -np.inf)
        with mp.Pool(processes=args.processes) as pool:
            # [MODIFIED] Unpack the new timing metrics
            for pop_idx, seed_idx, reward, t_phys, t_rend, t_rpc in tqdm(pool.imap_unordered(evaluate_individual, jobs), 
                                                                          total=len(jobs), desc=f"Gen {gen}"):
                results_matrix[pop_idx, seed_idx] = reward
                gen_time_physics += t_phys
                gen_time_render += t_rend
                gen_time_rpc += t_rpc

        # [ADDED] Print time profiling for the generation
        total_gen_time = gen_time_physics + gen_time_render + gen_time_rpc
        if total_gen_time > 0:
            print(f"    [Time Profile] Physics: {gen_time_physics:.1f}s ({(gen_time_physics/total_gen_time)*100:.1f}%) "
                  f"| Render: {gen_time_render:.1f}s ({(gen_time_render/total_gen_time)*100:.1f}%) "
                  f"| RPC+YOLO: {gen_time_rpc:.1f}s ({(gen_time_rpc/total_gen_time)*100:.1f}%)")
    ...
    try:
        with mp.Pool(processes=args.processes) as pool:
            # [MODIFIED] Ensure correct unpacking for final evaluation too
            results_iterator = tqdm(pool.imap_unordered(evaluate_individual, final_jobs), total=len(final_jobs), desc="Final Eval")
            for pop_idx, seed_idx, reward, t_phys, t_rend, t_rpc in results_iterator:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--global-seed", type=int, default=42) # This will be overridden in the loop
    parser.add_argument("--outdir", default="runs_cartpole")
    parser.add_argument("--checkpoint-freq", type=int, default=5)

    args = parser.parse_args()
    
    # --- MODIFIED: Multi-Run Loop for Seeds 101-105 ---
    seeds_to_run = [101, 102, 103, 104, 105]
    
    print(f"Starting Batch Experiment: {len(seeds_to_run)} runs.")
    print(f"Seeds: {seeds_to_run}")
    
    for run_idx, seed in enumerate(seeds_to_run):
        print(f"\n{'='*60}")
        print(f"STARTING RUN {run_idx+1}/{len(seeds_to_run)} | SEED: {seed}")
        print(f"{'='*60}\n")
        ...
        print(f"\n{'='*60}")
        print(f"FINISHED RUN {run_idx+1}/{len(seeds_to_run)} | SEED: {seed}")
        print(f"{'='*60}\n")