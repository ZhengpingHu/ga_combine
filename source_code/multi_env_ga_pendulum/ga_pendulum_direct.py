#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Pendulum GA Client: Direct Env Input (3D) + Native Reward Shaping + MLP (Hidden Layer)
# Modified for Multi-Run (Seeds 101-105) with OMP Fix

import os
# [关键修复 1] 必须在 import torch 之前设置，解决 OMP Error #15
...
# [关键修复 2] 使用您指定的带有隐藏层的网络结构
# 这对于解决 Pendulum 非线性控制至关重要
class PendulumNNPolicy(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(16, 1),   # Hidden 16 -> Output 1
            nn.Tanh()           # Range [-1, 1]
        )
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Pendulum action range is [-2.0, 2.0]
...
def run_ga(args):
    # Set the global seed for this run
    if args.global_seed: set_global_seed(args.global_seed)
    
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    # [关键功能] 文件夹名包含 seed，避免冲突
    run_dir = os.path.join(args.outdir, f"ga_pendulum_direct_{ts}_seed{args.global_seed}")
    ...
    print(f"Training Pendulum Direct Env (Input 3). Global Seed: {args.global_seed}. Logs: {run_dir}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--global-seed", type=int, default=42) # Loop will override this
    parser.add_argument("--outdir", default="runs_pendulum_direct")
    ...
    for run_idx, seed in enumerate(seeds_to_run):
        print(f"\n{'='*60}")
        print(f"STARTING RUN {run_idx+1}/{len(seeds_to_run)} | GLOBAL SEED: {seed}")
        print(f"{'='*60}\n")
        ...
        print(f"\n{'='*60}")
        print(f"FINISHED RUN {run_idx+1}/{len(seeds_to_run)} | GLOBAL SEED: {seed}")
        print(f"{'='*60}\n")
