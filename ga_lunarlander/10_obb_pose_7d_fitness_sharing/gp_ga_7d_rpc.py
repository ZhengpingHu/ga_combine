#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Final Integrated Version: Competitive Fitness + Dynamic Seed Refresh
...
class SeedPortfolioManager:
    ...
    def update_and_refresh(self, results_matrix: np.ndarray, refresh_frac: float, refresh_direction: str, success_threshold: float):
        self.scheduler.update_after_generation(self.active_subset)
        if refresh_frac <= 0 or refresh_direction == 'none': return

        num_to_replace = int(np.floor(len(self.active_subset) * refresh_frac))
        if num_to_replace == 0: return

        # Use fitness sharing weights to evaluate seed difficulty
        ...
        if refresh_direction == 'bottom':
            indices_to_replace = sorted_indices_by_difficulty[-num_to_replace:]
            print(f"Replacing {num_to_replace} hardest performing seeds (highest weights).")
        elif refresh_direction == 'top':
            indices_to_replace = sorted_indices_by_difficulty[:num_to_replace]
            print(f"Replacing {num_to_replace} easiest performing seeds (lowest weights).")
        else:
            return
        ...
        if len(candidate_pool) < num_to_replace:
            print("[WARN] Not enough unused seeds to perform replacement.")
            return
            
        new_seeds = random.sample(candidate_pool, num_to_replace)
        
        # Replace seeds in the active subset
        for i_replace, i_new in zip(indices_to_replace, range(num_to_replace)):
...
# ----------------------------
# 6. Competitive Fitness Function
# ----------------------------
...
def run_ga(population_size, generations, elite_frac, sigma, processes,
    ...
    for gen in range(1, generations + 1):
        ...
        portfolio.update_and_refresh(results_matrix, seed_refresh_frac, seed_refresh_direction, success_threshold)
        ...
        elite_idx = np.argsort(competitive_scores)[-elite_num:]
        ...
        if best_reward > best_global_reward:
            ...
            print(f"💾 [SAVE] New best model to {run_dir} (avg reward {best_global_reward:.2f})")
...
# ----------------------------
def main():
    parser = argparse.ArgumentParser(description="GA Client with Competitive Fitness and Dynamic Seed Refresh")
    # RPC
    ...
    # Seed Portfolio
    parser.add_argument("--pool-size", type=int, default=64, help="Total size of the master seed pool.")
    ...
    # Seed Refresh
    parser.add_argument("--seed-refresh-frac", type=float, default=0.0, help="Fraction of seed subset to refresh each gen (0.0 to disable).")
    parser.add_argument("--seed-refresh-direction", type=str, default="none", choices=['none', 'top', 'bottom'], help="Refresh top (easiest) or bottom (hardest) seeds.")
    # Competitive Fitness
    ...
    print(f"🚀 [INFO] Starting GA with Competitive Fitness & Seed Refresh strategy")
    print(f"📡 Connecting to RPC server at {args.rpc_host}:{args.rpc_port}")

    # [MODIFIED] Explicitly map arguments to fix the TypeError
    run_ga(
