#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Final NEAT Version: v12 (Fixes JSON TypeError on tuple keys)
...
# [MODIFIED] 导入本地的 visualize.py
try:
    import visualize 
    VISUALIZE_ENABLED = True
except ImportError as e:
    print("="*50)
    print("[WARN] 本地的 'visualize.py' 导入失败或其依赖库 'graphviz' 丢失。")
    print("         请确保 'visualize.py' 与本脚本在同一文件夹中。")
    print("         并请在 (neat) 环境中运行: pip install graphviz")
    print(f"         详细错误: {e}")
    print("         将禁用网络可视化功能。")
    print("="*50)
    ...
    print("="*50)
    print("[WARN] Graphviz 可执行文件 (dot.exe) 未找到。")
    print("         请确保你已从 graphviz.org 下载并安装了 Graphviz，")
    print("         并在安装时勾选了 'Add to PATH' 并重启了终端。")
    print(f"         详细错误: {e}")
    print("         将禁用网络可视化功能。")
    print("="*50)
    VISUALIZE_ENABLED = False
...
# ----------------------------
# 4. NEAT Evaluation Function (Worker) (无变动)
# ----------------------------
...
def save_best_genome(run_dir: str, genome: neat.DefaultGenome, config: neat.Config, suffix: str = ""):
    ...
    if VISUALIZE_ENABLED:
        try:
            ...
            dot = visualize.draw_net(config, genome, view=False, filename=gv_path,
                                     node_names=node_names, show_disabled=True)
            
            img_path = os.path.join(run_dir, f"best_genome_net{suffix}")
            dot.format = 'png'
            dot.render(filename=img_path, view=False, cleanup=True)
            dot.save(gv_path) # 重新保存 .gv 源文件
                     
            print(f"  ...Network visualization saved to .gv and .png.")
        except Exception as e:
            print(f"  ...[WARN] 可视化失败 (Graphviz 已导入但 'dot' 命令执行失败): {e}")
    else:
        if "save_best_genome.warned_once" not in globals():
            print("  ...Skipping network visualization (dependencies failed to import).")
            globals()["save_best_genome.warned_once"] = True
...
def run_neat(processes,
    ...
    # cfg['neat_config'] = {s: dict(config.genome_config.neat_config[s]) for s in config.genome_config.neat_config.sections()}
    ...
    def eval_genomes(genomes, config):
        ...
        smoothed_scores = np.zeros(len(genome_list))
        new_history: Dict[int, List[float]] = {} 

        for i, (genome_id, genome) in enumerate(genome_list):
            past_scores = fitness_history.get(genome_id, [])
            ...
            new_history[genome_id] = new_score_history
        ...
        elite_num = max(2, int(survival_frac * len(genome_list)))
        elite_indices = np.argsort(smoothed_scores)[-elite_num:]
        avg_top_reward = float(raw_avg_rewards[elite_indices].mean())
        
        best_individual_idx = np.argmax(smoothed_scores)
        ...
        print(f"\n🏆 [GEN {gen:03d}] BestAvgReward(Smooth.Winner)={best_reward:+.2f}  AvgTop{elite_num}(Smooth.Elite)={avg_top_reward:+.2f}  WorstAvgReward={worst_reward:+.2f}")
        ...
        plot_reward_curve(run_dir, curve_gens, curve_best, curve_avg, curve_worst, survival_frac)
        ...
        if best_reward > best_global_reward:
            ...
            best_genome_this_gen = genome_list[best_individual_idx][1]
    ...
    try:
        with mp.Pool(processes=processes) as pool:
            ...
            for genome_id, seed_idx, reward in results_iterator:
                pop_idx = genome_id_to_pop_idx_map.get(genome_id)
