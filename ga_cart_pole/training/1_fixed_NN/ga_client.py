...
# ----------------------------
# 2. Neural Network (Fixed Linear Topology)
# ----------------------------
class FixedNNPolicy(nn.Module):
    def __init__(self):
        super().__init__()
        # CartPole: 4 inputs -> 2 outputs (Linear, No Hidden Layer)
...
class SeedPortfolioManager:
    ...
    # [LOGIC] Refresh based on Total Score (Inverse Difficulty)
    def update_and_refresh(self, results_matrix: np.ndarray, refresh_frac: float, refresh_direction: str, max_seed_age: int):
        ...
        # 2. Performance-based (Score-based Refresh)
        # seed_total_scores: 每个种子获得的总分
        # Low Score = Hard Seed (Hard to get points)
        # High Score = Easy Seed (Easy to get points)
        ...
        if num_perf_replace > 0 and refresh_direction != 'none':
            # Sort from Low (Hard) to High (Easy)
            sorted_indices = np.argsort(seed_total_scores) 
            
            perf_indices = set()
            
            if refresh_direction == 'bottom':
                # [FIXED] User Intent: Remove Hardest (Lowest Scores)
                # Take the start of the sorted list
                indices = sorted_indices[:num_perf_replace] 
                perf_indices.update(indices)
                # print(f"[DEBUG] Refreshing Bottom (Hardest) seeds.")
                
            elif refresh_direction == 'top':
                # [FIXED] User Intent: Remove Easiest (Highest Scores)
                # Take the end of the sorted list
                indices = sorted_indices[-num_perf_replace:]
                perf_indices.update(indices)
                # print(f"[DEBUG] Refreshing Top (Easiest) seeds.")
...
def evaluate_individual(args):
    ...
    try:
        ...
        return pop_idx, seed_idx, 0.0

# ----------------------------
# 6. Fitness Logic (PROPORTIONAL SHARING from DOC PAGE 13)
# ----------------------------
def calculate_competitive_fitness(results_matrix: np.ndarray) -> np.ndarray:
    """
    [DOC COMPLIANT] Proportional Fitness Sharing[cite: 161].
    Formula: G'(i, k) = G(i, k) / SUM_j(G(j, k))
    
    Logic:
    1. G(i, k) is the Raw Reward of individual i on seed k.
    2. SUM_j(G(j, k)) is the Total Reward of the population on seed k.
    3. Fitness(i) = Sum over k of (Your Score / Total Score).
    """
    # Ensure rewards are non-negative (CartPole is naturally >= 0, but safety first)
    # If rewards can be negative, shift them to be positive
    if np.min(results_matrix) < 0:
        results_matrix = results_matrix - np.min(results_matrix)

    # 1. Calculate the Denominator: Total score of population per seed (Column Sum)
    # [cite: 162] "SUM_K G(m1, mj; pk)" interpreted as Sum over population
    total_rewards_per_seed = results_matrix.sum(axis=0)
    
    # Avoid division by zero (if a seed kills everyone instantly)
    total_rewards_per_seed[total_rewards_per_seed == 0] = 1e-9
    
    # 2. Calculate Shared Fitness Matrix
    # Broadcast division: Each element divided by its column sum
    # Matrix shape: [Pop, Seeds]
    shared_fitness_matrix = results_matrix / total_rewards_per_seed
    
    # 3. Sum over seeds to get individual fitness
    # [cite: 160] Fitness sharing sums the shared values
...
def run_ga(args):
    ...
    run_dir = os.path.join(args.outdir, f"ga_cartpole_{ts}")
    ...
    for gen in range(1, args.generations + 1):
        ...
        # 1. Update & Refresh
        # Note: removed success_threshold dependency for refresh too, using total scores now
        ...
        # 3. Calc Fitness (DOC COMPLIANT SHARING)
